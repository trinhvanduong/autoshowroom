...
jQuery(document).ready(function(){
    jQuery("#vehicle").tzPortfolioPlusIsotope({
        "params": ' . $this->params . '        
    });
});
');
...
<?php if ($this->items): ?>
    ...
    <div id="TzContent" class="tzpp_bootstrap3 <?php echo $this->pageclass_sfx; ?>">
                ...
                <div class="option-combo">
                    ...
                    <div id="filter" class="option-set clearfix" data-option-key="filter">
                        <a href="#show-all" data-option-value="*"
                           class="btn btn-default btn-sm selected"><?php echo JText::_('COM_TZ_PORTFOLIO_PLUS_SHOW_ALL'); ?></a>
                            ...
                            switch ($sortfield):
                                case 'title':
                                    ...
                                    <a class="btn btn-default btn-sm<?php echo ($sort == 'alpha' || $sort == 'ralpha') ? ' selected' : '' ?>"
                                       href="#title"
                                       data-option-value="name"><?php echo JText::_('COM_TZ_PORTFOLIO_PLUS_TITLE'); ?></a>
                                    <?php
                                    break;
                                case 'date':
                                    ?>
                                    <a class="btn btn-default btn-sm<?php echo ($sort == 'date' || $sort == 'rdate') ? ' selected' : '' ?>"
                                       href="#date"
                                       data-option-value="date"><?php echo JText::_('COM_TZ_PORTFOLIO_PLUS_DATE'); ?></a>
                                    <?php
                                    break;
                                case 'hits':
                                    ?>
                    ...
                    <div id="layouts" class="option-set clearfix" data-option-key="layoutMode">
                        ...
                        if (count($params->get('layout_type', array('masonry', 'fitRows', 'straightDown'))) > 0):
                            foreach ($params->get('layout_type', array('masonry', 'fitRows', 'straightDown')) as $i => $param):
                                ?>
                                <a class="btn btn-default btn-sm<?php if ($i == 0) echo ' selected'; ?>"
                                   href="#<?php echo $param ?>" data-option-value="<?php echo $param ?>">
                                    <?php echo $param ?>
                                </a>
                            <?php endforeach; ?>
                        <?php endif; ?>
        ...
        <div id="vehicle" class="super-list variable-sizes clearfix tz__vehicles" itemscope
