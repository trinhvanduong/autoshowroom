...
if (!empty($this->filters)) {
    $main = $this->params->get('main_div_name');
    $doc = JFactory::getDocument();
    $doc->addStyleDeclaration('.hikashop_filter_main_div_'.$main.' .hikashop_filter_main:nth-child('.$this->maxColumn.'n+1){ clear:both;}');
    $count = 0;
    $filterActivated = false;
    $widthPercent = (100 / $this->maxColumn) - 0.1;
    ...
    <div class="hikashop_filter_main_div hikashop_filter_main_div_<?php echo $this->params->get('main_div_name'); ?>">
        ...
        <fieldset class="hikashop_filter_fieldset">
            ...
            <form action="<?php echo $url; ?>" method="post"
                ...
                <?php while ($count < (int)$this->maxFilter) {
                    $height = '';
    ...
    <?php }
    if ($this->buttonPosition != 'inside') {
        $style = 'style="margin-right:10px; display: block;"'; ?>
        <?php if ($this->buttonPosition == 'right') {
            $style = 'style="float:right; margin-left:10px;"';
        } ?>
        <?php if ($this->showButton) {
            echo '<span class="hikashop_filter_button_outside" ' . $style . '>';
            echo $this->cart->displayButton(JText::_('FILTER'),
                'filter',
                $this->params,
                $url,
                'document.getElementById(\'hikashop_filtered_' . $this->params->get('main_div_name') . '\').value=\'1\';document.forms[\'hikashop_filter_form_' . $this->params->get('main_div_name') . '\'].submit(); return false;',
                'id="hikashop_filter_button_' . $this->params->get('main_div_name') . '"', '', '',
                'btn');
            echo '</span>';
        } ?>
        <?php if ($this->showResetButton) {
            echo '<span class="hikashop_reset_button_outside" ' . $style . ' >';
            echo $this->cart->displayButton(JText::_('RESET'),
                'reset_filter',
                $this->params,
                $url,
                'document.getElementById(\'hikashop_reseted_' . $this->params->get('main_div_name') . '\').value=\'1\';document.forms[\'hikashop_filter_form_' . $this->params->get('main_div_name') . '\'].submit(); return false;',
                'id="hikashop_reset_button_' . $this->params->get('main_div_name') . '"','','',
                'btn');
            echo '</span>';
        } ?>
    <?php } ?>
