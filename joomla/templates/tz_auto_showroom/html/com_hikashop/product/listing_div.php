...
$pagination = $this->config->get('pagination', 'bottom'); ?>
    <div class="pagination_block">
        <?php if (in_array($pagination, array('top', 'both')) && $this->params->get('show_limit') && $this->pageInfo->elements->total && !$infinite_scroll) {
            $this->pagination->form = '_top';
            ?>
            <form action="<?php echo hikashop_currentURL(); ?>" method="post"
                  name="adminForm_<?php echo $mainDivName . $this->category_selected; ?>_top">
                <div class="hikashop_products_pagination hikashop_products_pagination_top">
                    <?php if ((int)$this->pagination->total > 1) {
                        echo $this->pagination->getListFooter($this->params->get('limit'));
                    } ?>
                    <span class="hikashop_results_counter"><?php echo $this->pagination->getResultsCounter(); ?></span>
                </div>
                <input type="hidden" name="filter_order_<?php echo $mainDivName . $this->category_selected; ?>"
                       value="<?php echo $this->pageInfo->filter->order->value; ?>"/>
                <input type="hidden" name="filter_order_Dir_<?php echo $mainDivName . $this->category_selected; ?>"
        ...
        <div class="tzview-style">
            ...
            <div class="switchToGrid">
                <i class="fa fa-th" data-toggle="tooltip" data-placement="top"
                   title="<?php echo JText::_('STYLE_GIRD') ?>"></i>
            </div>
            <div class="switchToList">
                <i class="fa fa-list" data-toggle="tooltip" data-placement="top"
                   title="<?php echo JText::_('STYLE_LIST') ?>"></i>
            </div>
        </div>
    </div>
<?php $attributes = ($columns > 1 && $this->params->get('consistencyheight', 1)) ? ' data-consistencyheight=".hikashop_subcontainer"' : '';
...
if (in_array($pagination, array('bottom', 'both')) && $this->params->get('show_limit') && $this->pageInfo->elements->total && !$infinite_scroll) {
    ...
    ?>
    <form action="<?php echo hikashop_currentURL(); ?>" method="post"
          name="adminForm_<?php echo $mainDivName . $this->category_selected; ?>_bottom">
